...
function reboot_prompt {
    ...
    if ($confirmation -ieq 'reboot now' -or $confirmation -ieq 'reboot continue') {
        ...
        elseif ($confirmation -ieq 'reboot continue') {
            ...
            start wt -p windows cmd.exe /c echo 'please confirm administrator access to launch wsl devel' & powershell.exe start-process -filepath powershell.exe -Verb RunAs -NoExit -ArgumentList '-Command', '$($env:USERPROFILE)\dvlp.ps1 $($global:devel_spawn_args)' >> `"$env:TEMP\spawnlogs.txt`" 2>&1
...
function remove_installation {
    ...
    Remove-Item "$env:USERPROFILE/repos/$($env:KINDTEK_WIN_GIT_OWNER)" -Recurse -Confirm -Force
    Remove-Item "$env:USERPROFILE/kache" -Recurse -Confirm -Force 
