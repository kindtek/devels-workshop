...
function get_wsl_distro_list {
    ...
    if ($distro_array.length -gt 1) {    
        for ($i = 0; $i -le $distro_array.length - 1; $i++) {
            if ($($distro_array[$i]) -ne "docker-desktop" -And $($distro_array[$i]) -ne "docker-desktop-data" -And $($distro_array[$i]) -ne "$env:KINDTEK_FAILSAFE_WSL_DISTRO" -and !([string]::IsNullOrWhiteSpace($($distro_array[$i]))) -and !([string]::IsNullOrEmpty($($distro_array[$i])))) {
